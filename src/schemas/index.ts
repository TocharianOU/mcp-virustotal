...
/**
 * File relationship type. Uses z.string() + refinement to avoid TypeScript
 * exceeding instantiation-depth limits on the 41-member enum tuple.
 */
...
export type FileRelationship = (typeof FILE_RELATIONSHIPS)[number];
...
export const GetFileRelationshipArgsSchema = z.object({
  ...
  relationship: z
    .string()
    .refine((v): v is FileRelationship => (FILE_RELATIONSHIPS as readonly string[]).includes(v), {
      message: `Must be one of: ${FILE_RELATIONSHIPS.join(', ')}`,
    })
    .describe('Type of relationship to query'),
  limit: z.number().min(1).max(40).optional(),
  cursor: z.string().optional(),
...
export type DomainRelationship = (typeof DomainRelationships)[number];
...
export const GetDomainRelationshipArgsSchema = z.object({
  ...
  relationship: z
    .string()
    .refine((v): v is DomainRelationship => (DomainRelationships as readonly string[]).includes(v), {
      message: `Must be one of: ${DomainRelationships.join(', ')}`,
    })
    .describe('Type of relationship to query'),
  limit: z.number().min(1).max(40).optional(),
  cursor: z.string().optional(),
